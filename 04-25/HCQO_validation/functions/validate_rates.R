...
validate_rates <- function(df, source_file_name) {
  ...
  if (!("NUM" %in% unique(df$UNIT)) || !("DEN" %in% unique(df$UNIT))) {
    message("Validation Check 2 - Skipped: No numerators or denominators found.")
    return(df)
  }
  ...
  if (nrow(rate_identifiers) == 0) {
    message(paste("Validation Check 2 - Skipped: No RATE values found."))
    return(df)
  }
  ...
  if (nrow(invalid_rates) > 0) {
    ...
    message(paste("Validation Check 2 - Passed"))
  }
