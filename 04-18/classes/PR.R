...
InitializePR2 <- function(df_raw) {
  # Defining column and row names
  col.names <- c('65-74Y', '75-84Y', '85Y_OVER')
  row.names <- c('M', 'F','T')
  # Creating matrix
  number.of.cells <- length(col.names) * length(row.names)
...
InitializePR4 <- function(df_raw) {
  # Defining column and row names
  col.names <- c('18-49Y', '50-69Y', '70Y_OVER')
  row.names <- c('M', 'F','T')
  # Creating matrix
  number.of.cells <- length(col.names) * length(row.names)
...
InitializePR6 <- function(df_raw) {
  # Defining column and row names
  col.names <- c('65-69Y', '70-74Y', '75-79Y', '80-84Y', '85Y_OVER')
  row.names <- c('M', 'F','T')
...
setMethod("CalculateStdRate", "PR", function(object) {
  ...
  CalculateHelper <- function(row) {
    ...
    results_df$TIME_TIME_TIME_PERIOD <- row$TIME_PERIOD
  ...
  CalculateHelper_2 <- function(row) {
    ...
    results_df$SEX <- row$SEX
    results_df$REF_AREA <- row$REF_AREA
    results_df$TIME_TIME_PERIOD <- row$TIME_TIME_PERIOD
...
setMethod('PreprocessData','PR', function(object) {
  ...
  # Pivoting
  df_raw_sub <- df_raw_sub %>% tidyr::pivot_wider(id_cols = c(SEX,MEASURE, TIME_TIME_PERIOD, REF_AREA, AGE),
                                                  names_from = UNIT,
