...
source("functions/validate_names.R")
...
```{r}
# This section calls the load_csv function to load the data and displays basic information for inspection.
# ++ remove this chunk before distribution
# Load all CSV files specified in the data directory
file_paths <- list.files("data", pattern = "\\.csv$", full.names = TRUE, recursive = TRUE)
...
for (file_path in file_paths) {
    ...
    if (!is.null(df_raw)) {
        ...
        col_names_validated <- validate_names(df_raw, file_name)
        if (!col_names_validated) {
            next  # Skip further processing if column names are incorrect
        }
        
        # Continue with other validations
        validate_missing_denom(df_raw, file_name)
        validate_rates(df_raw, file_name)
