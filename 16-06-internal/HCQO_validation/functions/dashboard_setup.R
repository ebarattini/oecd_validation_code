...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("REF_AREA", "REF_AREA", choices = c("All" = "All", unique(viz_df$REF_AREA)), multiple = TRUE),  # Allow multi-select
...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    ...
    if (!is.null(input$REF_AREA) && !("All" %in% input$REF_AREA)) {
  ...
  output$graph <- renderPlot({
    ...
    # Add faceting to create panels for each REF_AREA (country)
    if (!is.null(input$REF_AREA) && !("All" %in% input$REF_AREA)) {
      p <- p + facet_wrap(~ REF_AREA)
    }
...
# Call the shiny app to display the dashboard
