...
# this function calculates crude and standardized rates as well as lower and upper confidence bounds

####################################################################################################


calculate_indicator <- function(raw_file) {
  ...
  if (is.null(df_raw)) {
    stop(message(paste("Failed to load data from:", raw_file)))
  }
