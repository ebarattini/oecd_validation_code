...
indicator_choices <- c(
  ...
  # PS indicators
  "FORBPROC - Retained surgical item or unretrieved device fragment using linked data" = "FORBPROC",
  "POSTPESP - Postoperative pulmonary embolism - hip and knee replacement discharges using linked data" = "POSTPESP",
  "POSTDVSP - Postoperative deep vein thrombosis - hip and knee replacement discharges using linked data" = "POSTDVSP",
  "POSTSESP - Postoperative sepsis - abdominal discharges using linked data" = "POSTSESP",
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("REF_AREA", "REF_AREA", choices = c("All" = "All", unique(viz_df$REF_AREA)), multiple = TRUE),
...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    ...
    if (!"All" %in% input$REF_AREA) {
      data <- data %>% filter(REF_AREA %in% input$REF_AREA)
    }
  ...
  output$graph <- renderPlot({
    data <- filtered_data()
    y_max <- max(data$Rate, na.rm = TRUE) + 0.05 * max(data$Rate, na.rm = TRUE)
    y_limits <- c(0, y_max)
    
    p <- ggplot(data, aes(x = TIME_PERIOD, y = Rate, group = interaction(SEX, REF_AREA), color = SEX)) +
      scale_y_continuous(limits = y_limits) +
      labs(x = "Years", y = "Rate", title = paste("Graph for", input$indicator)) +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5),
            axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
            axis.text.y = element_text(size = 12),
            strip.text = element_text(size = 16, face = "bold"))  # Increase the size and boldness of facet titles
    ...
    if (input$graphType == "bar") {
      ...
      p <- p + facet_wrap(~REF_AREA, scales = "free_y")  # Facet by country, allow y-axis to scale independently
    } else {  # For line graph
      p <- p + geom_line() +
